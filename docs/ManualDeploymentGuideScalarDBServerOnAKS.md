...
* Follow [Set up a database guide](./SetupDatabase.md) to set up a database for Scalar DB. Please avoid the `Scale Performance` section in this guide as it is possible to set Request Units(RU) value when creating the schema for the scalar DB server.

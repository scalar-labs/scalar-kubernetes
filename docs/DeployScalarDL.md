...
if the internal_domain var is not correct or the cassandra is not fully start, the schema loading job can fail, you will have the following error
...
FAILED - RETRYING: Check Schema Loading job have been successful (8 retries left).
FAILED - RETRYING: Check Schema Loading job have been successful (7 retries left).
FAILED - RETRYING: Check Schema Loading job have been successful (6 retries left).
FAILED - RETRYING: Check Schema Loading job have been successful (5 retries left).
FAILED - RETRYING: Check Schema Loading job have been successful (4 retries left).
FAILED - RETRYING: Check Schema Loading job have been successful (3 retries left).
...
fatal: [bastion-example-k8s-azure-b8ci1si.eastus.cloudapp.azure.com]: FAILED! => {"attempts": 10, "changed": false, "resources": [{"apiVersion": "batch/v1", "kind": "Job", "metadata": {"annotations": {"meta.helm.sh/release-name": "load-schema", "meta.helm.sh/release-namespace": "default"}, "creationTimestamp": "2020-08-13T05:18:57Z", "labels": {"app.kubernetes.io/instance": "load-schema", "app.kubernetes.io/managed-by": "Helm", "app.kubernetes.io/name": "schema-loading", "app.kubernetes.io/version": "1.0.0", "helm.sh/chart": "schema-loading-1.0.0"}, "name": "load-schema-schema-loading-cassandra", "namespace": "default", "resourceVersion": "5538", "selfLink": "/apis/batch/v1/namespaces/default/jobs/load-schema-schema-loading-cassandra", "uid": "78d3943c-8006-4f46-8ec5-7a9790fb0569"}, "spec": {"backoffLimit": 0, "completions": 1, "parallelism": 1, "selector": {"matchLabels": {"controller-uid": "78d3943c-8006-4f46-8ec5-7a9790fb0569"}}, "template": {"metadata": {"creationTimestamp": null, "labels": {"app.kubernetes.io/instance": "load-schema", "app.kubernetes.io/name": "schema-loading", "controller-uid": "78d3943c-8006-4f46-8ec5-7a9790fb0569", "job-name": "load-schema-schema-loading-cassandra"}}, "spec": {"containers": [{"command": ["/bin/bash", "-c", "echo applying Scalar DL schema && dockerize --template create_schema.cql.tmpl:create_schema.cql && cqlsh --cqlversion=3.4.4 ${SCALAR_DB_CONTACT_POINTS} -u ${SCALAR_DB_USERNAME} -p ${SCALAR_DB_PASSWORD} -f create_schema.cql"], "env": [{"name": "SCALAR_DB_CONTACT_POINTS", "value": "cassandra-lb.internal.scalar-labs.com"}, {"name": "SCALAR_DB_CONTACT_PORT", "value": "9042"}, {"name": "SCALAR_DB_USERNAME", "value": "cassandra"}, {"name": "SCALAR_DB_PASSWORD", "value": "cassandra"}, {"name": "CASSANDRA_REPLICATION_FACTOR", "value": "3"}], "image": "scalarlabs/scalar-ledger:2.0.7", "imagePullPolicy": "IfNotPresent", "name": "schema-import", "resources": {}, "terminationMessagePath": "/dev/termination-log", "terminationMessagePolicy": "File"}], "dnsPolicy": "ClusterFirst", "imagePullSecrets": [{"name": "reg-docker-secrets"}], "restartPolicy": "Never", "schedulerName": "default-scheduler", "securityContext": {}, "terminationGracePeriodSeconds": 30}}}, "status": {"conditions": [{"lastProbeTime": "2020-08-13T05:19:34Z", "lastTransitionTime": "2020-08-13T05:19:34Z", "message": "Job has reached the specified backoff limit", "reason": "BackoffLimitExceeded", "status": "True", "type": "Failed"}], "failed": 1, "startTime": "2020-08-13T05:18:57Z"}}]}
```

Ensure that the cassandra is fully started and ready and re-apply the `ansible-playbook -i ${SCALAR_K8S_CONFIG_DIR}/inventory.ini operation/playbook-deploy-scalardl.yml`
